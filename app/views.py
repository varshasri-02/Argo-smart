...
def visitor_find_soil(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            region = request.POST.get('region')  # FIXED: Get region from POST
            
            # FIXED: Filter by both district and region if region is provided
            if region:
                soils = SoilLocationDetail.objects.filter(
                    district=district, 
                    region=region,
                    status=True
                ).select_related('district', 'region')
            else:
                # If no region selected, show all soils for the district
                soils = SoilLocationDetail.objects.filter(
                    district=district, 
                    status=True
                ).select_related('district', 'region')
            
            # FIXED: Add message if no data found
            if not soils.exists():
                try:
                    district_name = District.objects.get(id=district).name
                    messages.info(request, f'No soil data available for {district_name}')
...
#<-----Find Soil Detail for Visitor - FIXED----->#
@login_required(login_url='visitor_login')
@user_passes_test(is_visitor)
def visitor_find_soil_detail(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            soil = request.POST.get('soil')  # FIXED: Use .get() instead of []
            soils = SoilDetail.objects.filter(soil=soil).select_related('soil')
            
            # FIXED: Check if data exists
            if not soils.exists():
                try:
                    soil_name = Soil.objects.get(id=soil).name
                    messages.info(request, f'No details available for {soil_name}')
                except Soil.DoesNotExist:
                    messages.error(request, 'Invalid soil type selected')
    else:
        form = FindSoilDetailForm()
    return render(request, 'visitor/visitor_find_soil_detail.html', {'form': form, 'soils': soils})

#<-----Find Rainfall for Visitor - FIXED----->#
@login_required(login_url='visitor_login')
@user_passes_test(is_visitor)
def visitor_find_rainfall(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            district = request.POST.get('district')  # FIXED: Use .get() instead of []
            # FIXED: Order by year in ascending order with proper joins
            rainfalls = RainfallDetail.objects.filter(
                district=district
            ).select_related('year', 'district').order_by('year__name')
            
            # FIXED: Check if data exists
            if not rainfalls.exists():
                try:
                    district_name = District.objects.get(id=district).name
                    messages.info(request, f'No rainfall data available for {district_name}')
                except District.DoesNotExist:
                    messages.error(request, 'Invalid district selected')
    else:
...
#<-----Visitor Virtual Market Vegetable category----->#
@login_required(login_url='visitor_login')
@user_passes_test(is_visitor)
...
def officer_signup(request):
    if request.method=='POST':
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            f2=form2.save(commit=False)
            f2.user=user
            f2.status=True  # Auto-approve new officers
            user2=f2.save()
...
def seller_signup(request):
    if request.method=='POST':
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            f2=form2.save(commit=False)
            f2.user=user
            f2.status=True  # Auto-approve new sellers
            user2=f2.save()
...
def load_regions(request):
    """AJAX view to load regions based on selected district"""
    district_id = request.GET.get('district_id')
    print(f"District ID received: {district_id}")  # Debug print
    
    regions = Region.objects.filter(district_id=district_id).order_by('name')
    print(f"Number of regions found: {regions.count()}")  # Debug print
    
    regions_list = list(regions.values('id', 'name'))
    print(f"Regions data: {regions_list}")  # Debug print
